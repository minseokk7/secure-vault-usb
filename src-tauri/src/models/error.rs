...
impl SecureVaultError {
    ...
    pub fn user_friendly_message(&self) -> String {
        match self {
            ...
            SecureVaultError::Auth(AuthError::InvalidInput(msg)) => msg.clone(),
    ...
    pub fn severity(&self) -> ErrorSeverity {
        match self {
            ...
            SecureVaultError::Database(DatabaseError::IntegrityCheckFailed) => {
...
impl VaultError {
    ...
    pub fn user_friendly_message(&self) -> String {
        match self {
            ...
            VaultError::AlreadyInitialized => "볼트가 이미 초기화되어 있습니다.".to_string(),
            ...
            VaultError::AccessDenied => "접근이 거부되었습니다. 권한을 확인해주세요.".to_string(),
            VaultError::LockFailed => "볼트 잠금에 실패했습니다.".to_string(),
            VaultError::UnlockFailed => "볼트 잠금 해제에 실패했습니다.".to_string(),
            VaultError::BackupFailed => "볼트 백업에 실패했습니다.".to_string(),
            VaultError::RestoreFailed => "볼트 복원에 실패했습니다.".to_string(),
            VaultError::InsufficientSpace(available) => {
                format!("저장 공간이 부족합니다. 사용 가능한 공간: {}MB", available)
            }
            VaultError::FileTooLarge { size, max_size } => {
                format!(
                    "파일 크기가 너무 큽니다. 현재: {}MB, 최대: {}MB",
                    size / (1024 * 1024),
                    max_size / (1024 * 1024)
                )
            }
            ...
            VaultError::OperationCancelled => "작업이 사용자에 의해 취소되었습니다.".to_string(),
        }
    }
}
