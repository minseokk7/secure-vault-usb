use crate::models::{
    error::VaultError,
    file::{calculate_file_hash, calculate_file_hash_parallel, FileEntry, FileSortBy},
};
use crate::services::{
    compression::CompressionService, crypto::CryptoService, database::DatabaseService,
};
use chrono::Utc;
use std::fs;
...
impl FileService {
    ...
    pub async fn initialize(
        ...
        // 디렉토리 생성
        fs::create_dir_all(&encrypted_files_path).map_err(|e| {
            VaultError::DatabaseError(format!("암호화 파일 디렉토리 생성 실패: {}", e))
        })?;
    ...
    pub fn set_vault_info(&mut self, vault_path: &str, master_key: [u8; 32]) {
        log::info!(
            "DEBUG: set_vault_info called. Setting Master Key0={:02X}",
            master_key[0]
        );
    ...
    fn ensure_initialized(&mut self) -> Result<(), VaultError> {
        ...
        // Clone()된 FileService는 DB 연결이 끊어져 있으므로 반드시 확인해야 함
        if !self.database_service.is_initialized() {
            let vault_path = self.vault_path.as_ref().unwrap();
    ...
    pub async fn add_file(
        ...
        if source_file_path.is_empty() {
            return Err(VaultError::DatabaseError(
                "원본 파일 경로가 유효하지 않습니다.".to_string(),
            ));
        }

        if vault_file_name.is_empty() {
            return Err(VaultError::DatabaseError(
                "볼트 파일명이 유효하지 않습니다.".to_string(),
            ));
        }
        ...
        if file_metadata.len() == 0 {
            return Err(VaultError::DatabaseError(
                "빈 파일은 추가할 수 없습니다.".to_string(),
            ));
        }

        if file_metadata.len() > 5 * 1024 * 1024 * 1024 {
            return Err(VaultError::DatabaseError(
                "파일 크기가 너무 큽니다. (최대 5GB)".to_string(),
            ));
        }
        ...
        log::info!(
            "DEBUG: Encrypting file with Master Key0={:02X}",
            master_key[0]
        );

        let encrypted_data = self
            .crypto_service
            .encrypt_data_csharp_compatible(&file_data, &master_key)
            .map_err(|e| {
                VaultError::DatabaseError(format!("파일 암호화 중 오류가 발생했습니다: {}", e))
            })?;

        if encrypted_data.is_empty() {
            return Err(VaultError::DatabaseError(
                "파일 암호화에 실패했습니다.".to_string(),
            ));
        }

        // 암호화된 파일 저장
        let encrypted_files_path = self
        ...
        // 중요: DB ID와 파일명 ID를 일치시킵니다.
        // FileEntry::new는 랜덤 ID를 생성하므로, 우리가 생성한 file_id로 덮어씌워야 합니다.
        // 그렇지 않으면 media.rs에서 DB ID로 파일을 찾을 때 파일이 존재하지 않는 오류가 발생합니다.
    ...
    /// 파일을 볼트에 추가합니다 (진행률 콜백 및 취소 지원).
    ///
    /// 대용량 파일의 경우 스트리밍 암호화를 사용하여 메모리 효율성을 보장하고,
    /// 청크 단위로 진행률을 업데이트합니다.
    ///
    /// # 매개변수
    /// * `source_file_path` - 원본 파일 경로
    /// * `vault_file_name` - 볼트 내 파일명
    /// * `folder_id` - 폴더 ID (루트는 None)
    /// * `cancellation_token` - 취소 토큰 (None이면 취소 불가)
    /// * `progress_callback` - 청크 처리 후 호출되는 콜백 (처리된 바이트, 전체 바이트)
    ///
    /// # 반환값
    /// * `Result<FileEntry, VaultError>` - 생성된 파일 엔트리
    pub fn add_file_with_progress<F>(
        &mut self,
        source_file_path: &str,
        vault_file_name: &str,
        folder_id: Option<Uuid>,
        cancellation_token: Option<&crate::services::upload_manager::CancellationToken>,
        progress_callback: F,
    ) -> Result<FileEntry, VaultError>
    where
        F: Fn(u64, u64), // (처리된 바이트, 전체 바이트)
    {
        ...
        if let Some(key) = self.master_key {
            log::info!("파일 추가 시 마스터 키: Key0={:02X}", key[0]);
        } else {
            log::error!("파일 추가 시 마스터 키가 설정되지 않음!");
        }

        // 스트리밍 암호화 + 진행률 콜백 사용
        let encrypted_size = self.encrypt_file_streaming_with_progress(
            source_path,
            &encrypted_file_path,
            cancellation_token,
            &progress_callback,
        ...
        // 해시 계산 (별도 패스 - 스트리밍 중에 처리하려면 더 복잡해짐)
        // 취소 체크
        if let Some(token) = cancellation_token {
            if token.is_cancelled() {
                // 암호화된 파일 삭제
        ...
        let checksum = {
            ...
            let file = fs::File::open(source_path).map_err(|e| {
                VaultError::DatabaseError(format!("해시 계산용 파일 열기 실패: {}", e))
            })?;
            let mut reader = BufReader::with_capacity(1 * 1024 * 1024, file);
            let mut hasher = Sha256::new();
            let mut buffer = vec![0u8; 1 * 1024 * 1024]; // 1MB 힙 할당 (스택 오버플로우 방지)

            loop {
                let bytes_read = reader.read(&mut buffer).map_err(|e| {
                    VaultError::DatabaseError(format!("해시 계산 중 읽기 실패: {}", e))
                })?;
                if bytes_read == 0 {
                    break;
                }
    ...
    pub async fn remove_file(&mut self, file_id: &Uuid) -> Result<(), VaultError> {
        ...
        // 메타데이터 조회하여 정확한 파일명 확인
        let file_entry_opt = self.database_service.get_file(file_id)?;

        // 파일이 존재하면 삭제
        if let Some(file_entry) = file_entry_opt {
            let encrypted_files_path = self
                .encrypted_files_path
                .as_ref()
                .ok_or(VaultError::NotInitialized)?;

            // DB에 저장된 실제 암호화 파일명 사용
            let encrypted_file_path = encrypted_files_path.join(&file_entry.encrypted_file_name);

            if encrypted_file_path.exists() {
                // 보안을 위해 파일을 0으로 덮어쓴 후 삭제
                self.secure_delete_file(&encrypted_file_path)?;
            } else {
                // 이전 방식(.enc 강제)으로도 확인 (하위 호환성)
                let fallback_path = encrypted_files_path.join(format!("{}.enc", file_id));
                if fallback_path.exists() {
                    self.secure_delete_file(&fallback_path)?;
                }
            }
        }
    ...
    pub fn encrypt_file_data(&self, data: &[u8]) -> Result<Vec<u8>, VaultError> {
        let master_key = self.master_key.ok_or(VaultError::NotInitialized)?;
        self.crypto_service
            .encrypt_data_csharp_compatible(data, &master_key)
    ...
    pub fn encrypt_file_parallel_streaming<P: AsRef<Path>>(
        &self,
        input_path: P,
        output_path: P,
        ...
        // 병렬 처리용 청크 크기 (20MB)
        const PARALLEL_CHUNK_SIZE: usize = 20 * 1024 * 1024;
        let num_chunks =
            ((total_size as usize + PARALLEL_CHUNK_SIZE - 1) / PARALLEL_CHUNK_SIZE).max(1);
        ...
        for chunk_idx in 0..num_chunks {
            ...
            let handle = thread::spawn(move || {
                let encrypted_chunk =
                    crypto_service.encrypt_data_csharp_compatible(&chunk_data, &master_key_copy)?;
        ...
        for handle in handles {
            handle
                .join()
                .map_err(|_| VaultError::DatabaseError("병렬 암호화 스레드 실패".to_string()))?
                .map_err(|e| e)?;
        }
        ...
        for (_, encrypted_chunk) in encrypted_chunks.iter() {
            ...
            // 암호화된 청크 데이터 저장
            writer.write_all(encrypted_chunk).map_err(|e| {
                VaultError::DatabaseError(format!("암호화된 청크 쓰기 실패: {}", e))
            })?;
    ...
    pub fn encrypt_file_streaming<P: AsRef<Path>>(
        ...
        loop {
            ...
            let encrypted_chunk = self
                .crypto_service
                .encrypt_data_csharp_compatible(&buffer[..bytes_read], &master_key)
            ...
            if chunk_counter % 25 == 0
                || (processed_bytes * 20 / total_size)
                    > ((processed_bytes - CHUNK_SIZE as u64) * 20 / total_size)
            {
    ...
    /// 파일을 스트리밍 방식으로 암호화합니다 (진행률 콜백 및 취소 지원).
    ///
    /// # 매개변수
    /// * `input_path` - 입력 파일 경로
    /// * `output_path` - 출력 파일 경로
    ...
    ///
    /// # 반환값
    /// * `Result<u64, VaultError>` - 암호화된 파일 크기 (취소 시 에러)
    pub fn encrypt_file_streaming_with_progress<P, F>(
        &self,
        input_path: P,
        output_path: P,
        cancellation_token: Option<&crate::services::upload_manager::CancellationToken>,
        ...
        F: Fn(u64, u64), // (처리된 바이트, 전체 바이트)
    {
        ...
        let master_key = self.master_key.ok_or(VaultError::NotInitialized)?;
        log::info!("스트리밍 암호화: 마스터 키 (Key0={:02X})", master_key[0]);
        ...
        loop {
            ...
            if chunk_counter % 10 == 0 {
                let progress_percent = (bytes_processed * 100 / total_size).min(100);
                log::info!(
                    "스트리밍 암호화 진행: {}% ({}/{}MB)",
                    progress_percent,
    ...
    pub async fn export_file(
        ...
        let master_key = self.master_key.ok_or(VaultError::NotInitialized)?;
        log::info!(
            "DEBUG: Export File Decryption - Master Key[0]: {:02X}",
            master_key[0]
        );
    ...
    pub async fn update_file(
        ...
        if new_content.is_empty() {
            return Err(VaultError::DatabaseError(
                "파일 내용이 비어있습니다.".to_string(),
            ));
        }

        // 기존 메타데이터 조회
        let mut file_entry = self.database_service.get_file(file_id)?.ok_or_else(|| {
            VaultError::DatabaseError(format!("파일 ID '{}'를 찾을 수 없습니다.", file_id))
        })?;
        ...
        let encrypted_data = self
            .crypto_service
            .encrypt_data_csharp_compatible(new_content, &master_key)
            .map_err(|e| VaultError::DatabaseError(format!("파일 암호화 실패: {}", e)))?;
    ...
    pub fn get_file_content(&mut self, file_id: &str) -> Result<Vec<u8>, VaultError> {
        ...
        let file_entry = self.database_service.get_file(&uuid)?.ok_or_else(|| {
            VaultError::DatabaseError("데이터베이스에서 파일을 찾을 수 없습니다.".to_string())
        })?;
        ...
        let securevault_dir = encrypted_files_path
            .parent()
            .unwrap_or(encrypted_files_path);
        log::info!(
            ".securevault 디렉토리 존재 여부: {}",
            securevault_dir.exists()
        );
        log::info!(
            "files 디렉토리 존재 여부: {}",
            encrypted_files_path.exists()
        );

        if !encrypted_file_path.exists() {
            // 파일이 존재하지 않으면 테스트용 더미 데이터 생성
            log::warn!(
                "암호화된 파일이 존재하지 않음, 테스트용 더미 데이터 반환: {}",
                file_id
            );
        ...
        // 실제 파일이 존재하면 복호화하여 반환
        log::info!("암호화된 파일 읽기 성공: {} bytes", encrypted_data.len());

        // 파일 복호화
        let master_key = self.master_key.ok_or(VaultError::NotInitialized)?;

        // 1. 청크 단위 복호화 시도 (업로드된 파일 형식)
        ...
        while cursor < encrypted_data.len() {
            // 헤더(길이) 읽기 (4바이트)
            if cursor + 4 > encrypted_data.len() {
                success = false;
                break;
            }

            let mut len_bytes = [0u8; 4];
            len_bytes.copy_from_slice(&encrypted_data[cursor..cursor + 4]);
            let chunk_len = u32::from_le_bytes(len_bytes) as usize;
            cursor += 4;

            // 데이터 읽기
            if cursor + chunk_len > encrypted_data.len() {
                success = false;
                break;
            }

            let chunk = &encrypted_data[cursor..cursor + chunk_len];
            cursor += chunk_len;

            // 청크 복호화
            match self
                .crypto_service
                .decrypt_data_csharp_compatible(chunk, &master_key)
            {
                Ok(data) => decrypted_buffer.extend_from_slice(&data),
                Err(_) => {
                    success = false;
                    break;
                }
        ...
        if success && !decrypted_buffer.is_empty() {
            log::info!("청크 단위 복호화 성공: {} bytes", decrypted_buffer.len());
            return Ok(decrypted_buffer);
        }

        // 2. 실패 시 단일 블록 복호화 시도 (기존/수정된 파일 형식)
        log::warn!(
            "청크 복호화 실패, 단일 블록 복호화 시도 (Legacy/Single): {}",
            file_id
        );
        ...
        log::info!("파일 복호화 성공: {} bytes", decrypted_data.len());
        Ok(decrypted_data)
    }
    ...
    pub fn update_file_content(
        ...
        log::info!(
            "파일 내용 업데이트 시작: file_id={}, content_size={}",
            file_id,
            content.len()
        );

        // 파일 메타데이터 조회
        let mut file_entry = self.database_service.get_file(&uuid)?.ok_or_else(|| {
            VaultError::DatabaseError(format!("파일 ID '{}'를 찾을 수 없습니다.", file_id))
        })?;
    ...
    fn secure_delete_file(&self, file_path: &Path) -> Result<(), VaultError> {
        ...
        for _pass in 0..3 {
            ...
            while written < file_size {
                let bytes_to_write =
                    std::cmp::min(buffer.len() as u64, file_size - written) as usize;
    ...
    /// 스트리밍 방식으로 암호화된 파일을 복호화합니다 (전체 파일 한번에 처리 - 레거시).
    ///
    /// # 매개변수
    ...
    pub async fn decrypt_file_streaming<P: AsRef<Path>>(
        &self,
        encrypted_file_path: P,
    ...
    /// 청크 기반 스트리밍 방식으로 암호화된 파일을 복호화합니다.
    /// 암호화 형식: [4바이트 청크 크기][암호화된 청크 데이터]...
    ///
    /// # 매개변수
    /// * `encrypted_file_path` - 암호화된 파일 경로
    ///
    /// # 반환값
    /// * `Result<Vec<u8>, VaultError>` - 복호화된 데이터
    pub fn decrypt_file_streaming_chunked<P: AsRef<Path>>(
        &self,
        encrypted_file_path: P,
    ) -> Result<Vec<u8>, VaultError> {
        use std::io::{BufReader, Read};
        ...
        let encrypted_file_path = encrypted_file_path.as_ref();
        log::info!("청크 스트리밍 복호화 시작: {:?}", encrypted_file_path);
        ...
        let master_key = self.master_key.ok_or(VaultError::NotInitialized)?;
        log::info!("복호화 마스터 키: Key0={:02X}", master_key[0]);

        let file = fs::File::open(encrypted_file_path)
        ...
        loop {
            ...
            if chunk_size == 0 || chunk_size > 100 * 1024 * 1024 {
                // 비정상적인 청크 크기 (100MB 초과)
                return Err(VaultError::DatabaseError(format!(
                    "비정상적인 청크 크기: {} bytes",
                    chunk_size
                )));
            ...
            reader.read_exact(&mut encrypted_chunk).map_err(|e| {
                VaultError::DatabaseError(format!("암호화된 청크 읽기 실패: {}", e))
            })?;

            // 청크 복호화
            let decrypted_chunk = self
                .crypto_service
                .decrypt_data_csharp_compatible(&encrypted_chunk, &master_key)
                .map_err(|e| {
                    VaultError::DatabaseError(format!("청크 {} 복호화 실패: {}", chunk_count, e))
                })?;
...
mod tests {
    ...
    async fn test_create_new_file() {
        ...
        let master_key = [0u8; 32]; // 테스트용 키
        file_service
            .initialize(vault_path, master_key)
            .await
            .unwrap();
    ...
    async fn test_file_operations() {
        ...
        let file_entry = file_service
            .create_new_file(None, file_name, content)
            .await
            .unwrap();
