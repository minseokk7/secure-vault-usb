use crate::models::compression::{CompressionLevel, CompressionResult, CompressionSettings, CompressionError};
...
impl CompressionService {
    ...
    pub fn compress_data(&self, data: &[u8], level: Option<CompressionLevel>) -> Result<(Vec<u8>, CompressionResult), CompressionError> {
        if data.is_empty() {
            return Err(CompressionError::CompressionFailed("빈 데이터는 압축할 수 없습니다.".to_string()));
        }
        ...
        let compressed_data = encoder.finish()
            .map_err(|e| CompressionError::CompressionFailed(format!("압축 완료 중 오류: {}", e)))?;
        ...
        if compressed_size >= original_size {
            log::debug!("압축 효과가 없어 원본 데이터를 반환합니다. 원본: {}바이트, 압축: {}바이트", 
                original_size, compressed_size);
            return Ok((data.to_vec(), result));
        }

        log::debug!("압축 완료: {}바이트 -> {}바이트 ({:.1}% 절약)", 
            original_size, compressed_size, result.space_saved_percent());
    ...
    pub fn decompress_data(&self, compressed_data: &[u8]) -> Result<Vec<u8>, CompressionError> {
        if compressed_data.is_empty() {
            return Err(CompressionError::DecompressionFailed("빈 데이터는 압축 해제할 수 없습니다.".to_string()));
        }
        ...
        decoder.read_to_end(&mut decompressed_data)
            .map_err(|e| CompressionError::DecompressionFailed(format!("압축 해제 중 오류: {}", e)))?;

        let decompression_time = start_time.elapsed().as_millis();
        log::debug!("압축 해제 완료: {}바이트 -> {}바이트 ({}ms)", 
            compressed_data.len(), decompressed_data.len(), decompression_time);
    ...
    pub fn compress_file_parallel_streaming<P: AsRef<std::path::Path>>(&self, input_path: P, output_path: P, file_extension: &str) -> Result<CompressionResult, CompressionError> {
        ...
        let file_size = std::fs::metadata(input_path)
            .map_err(|e| CompressionError::CompressionFailed(format!("입력 파일 정보 읽기 실패: {}", e)))?
            .len();
        ...
        if !self.should_compress(file_size, file_extension) {
            std::fs::copy(input_path, output_path)
                .map_err(|e| CompressionError::CompressionFailed(format!("파일 복사 실패: {}", e)))?;
            return Ok(CompressionResult::new(file_size, file_size, 0, self.settings.level));
        }
        ...
        let num_chunks = ((file_size as usize + PARALLEL_CHUNK_SIZE - 1) / PARALLEL_CHUNK_SIZE).max(1);
        let num_threads = std::cmp::min(num_chunks, num_cpus::get()).max(1);
        
        log::info!("병렬 압축: {} 청크, {} 스레드 사용", num_chunks, num_threads);
        ...
        for handle in handles {
            handle.join().map_err(|_| CompressionError::CompressionFailed("병렬 압축 스레드 실패".to_string()))?
                .map_err(|e| e)?;
        }
        ...
        let output_file = File::create(output_path)
            .map_err(|e| CompressionError::CompressionFailed(format!("출력 파일 생성 실패: {}", e)))?;
        ...
        for (_, compressed_chunk) in compressed_chunks.iter() {
            ...
            writer.write_all(&chunk_size.to_le_bytes())
                .map_err(|e| CompressionError::CompressionFailed(format!("청크 크기 쓰기 실패: {}", e)))?;
            
            // 압축된 청크 데이터 저장
            writer.write_all(compressed_chunk)
                .map_err(|e| CompressionError::CompressionFailed(format!("압축된 청크 쓰기 실패: {}", e)))?;
            
            total_compressed_size += 4 + compressed_chunk.len() as u64;
        }
        
        writer.flush()
            .map_err(|e| CompressionError::CompressionFailed(format!("파일 쓰기 완료 실패: {}", e)))?;
        
        let compression_time = start_time.elapsed().as_millis() as u64;
        let result = CompressionResult::new(file_size, total_compressed_size, compression_time, CompressionLevel::Fast);
        
        log::info!("병렬 압축 완료: {}MB -> {}MB ({:.1}% 절약, {}ms, {} 스레드)", 
                  file_size / (1024 * 1024),
    ...
    /// # 반환값
    /// * `Result<CompressionResult, CompressionError>` - 압축 결과
    pub fn compress_file_streaming<P: AsRef<std::path::Path>>(&self, input_path: P, output_path: P, file_extension: &str) -> Result<CompressionResult, CompressionError> {
        ...
        let input_file = File::open(input_path)
            .map_err(|e| CompressionError::CompressionFailed(format!("입력 파일 열기 실패: {}", e)))?;
        let output_file = File::create(output_path)
            .map_err(|e| CompressionError::CompressionFailed(format!("출력 파일 생성 실패: {}", e)))?;
        ...
        loop {
            ...
            encoder.write_all(&buffer[..bytes_read])
                .map_err(|e| CompressionError::CompressionFailed(format!("압축 쓰기 실패: {}", e)))?;
            
            total_read += bytes_read as u64;
            
            // 주기적으로 진행 상황 로그 (100MB마다)
            if total_read % (100 * 1024 * 1024) == 0 {
                log::info!("스트리밍 압축 진행: {}MB 처리 완료", total_read / (1024 * 1024));
            }
        }
        ...
        let compressed_size = std::fs::metadata(output_path)
            .map_err(|e| CompressionError::CompressionFailed(format!("출력 파일 정보 읽기 실패: {}", e)))?
            .len();
        
        let result = CompressionResult::new(file_size, compressed_size, compression_time, self.settings.level);
        
        // 압축 효과가 없는 경우 원본 파일로 교체
        if compressed_size >= file_size {
            log::debug!("스트리밍 압축 효과가 없어 원본 파일로 교체: {} -> {} bytes", file_size, compressed_size);
            std::fs::copy(input_path, output_path)
                .map_err(|e| CompressionError::CompressionFailed(format!("원본 파일 복사 실패: {}", e)))?;
            
            return Ok(CompressionResult::new(file_size, file_size, compression_time, self.settings.level));
        }
        
        log::info!("스트리밍 압축 완료: {} -> {} bytes ({:.1}% 절약, {}ms)", 
                  file_size, compressed_size, result.space_saved_percent(), compression_time);
    ...
    pub fn decompress_file_streaming<P: AsRef<std::path::Path>>(&self, input_path: P, output_path: P) -> Result<u64, CompressionError> {
        use std::io::{BufReader, BufWriter};
        use std::fs::File;
        ...
        // 파일 열기
        let input_file = File::open(input_path)
            .map_err(|e| CompressionError::DecompressionFailed(format!("압축 파일 열기 실패: {}", e)))?;
        let output_file = File::create(output_path)
            .map_err(|e| CompressionError::DecompressionFailed(format!("출력 파일 생성 실패: {}", e)))?;
        ...
        loop {
            let bytes_read = decoder.read(&mut buffer)
                .map_err(|e| CompressionError::DecompressionFailed(format!("압축 해제 읽기 실패: {}", e)))?;
            ...
            writer.write_all(&buffer[..bytes_read])
                .map_err(|e| CompressionError::DecompressionFailed(format!("파일 쓰기 실패: {}", e)))?;
            
            total_written += bytes_read as u64;
            
            // 주기적으로 진행 상황 로그 (100MB마다)
            if total_written % (100 * 1024 * 1024) == 0 {
                log::info!("스트리밍 압축 해제 진행: {}MB 처리 완료", total_written / (1024 * 1024));
            }
        }
        
        // 버퍼 플러시
        writer.flush()
            .map_err(|e| CompressionError::DecompressionFailed(format!("파일 쓰기 완료 실패: {}", e)))?;
        
        let decompression_time = start_time.elapsed().as_millis();
        log::info!("스트리밍 압축 해제 완료: {}MB ({}ms)", 
                  total_written / (1024 * 1024), decompression_time);
    ...
    pub fn compress_file_data(&self, data: &[u8], file_extension: &str) -> Result<(Vec<u8>, Option<CompressionResult>), CompressionError> {
        let file_size = data.len() as u64;

        // 압축 대상인지 확인
        if !self.should_compress(file_size, file_extension) {
            log::debug!("압축 대상이 아님: 크기={}바이트, 확장자={}", file_size, file_extension);
    ...
    pub fn calculate_compression_stats(&self, original_sizes: &[u64], compressed_sizes: &[u64]) -> CompressionStats {
...
mod tests {
    ...
    fn test_compression_levels() {
        ...
        // 각 압축 레벨 테스트
        let (fast_data, fast_result) = service.compress_data(data, Some(CompressionLevel::Fast)).unwrap();
        let (normal_data, normal_result) = service.compress_data(data, Some(CompressionLevel::Normal)).unwrap();
        let (max_data, max_result) = service.compress_data(data, Some(CompressionLevel::Maximum)).unwrap();
