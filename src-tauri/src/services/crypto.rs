...
use crate::models::{
    CryptoError, EncryptedData, EncryptionAlgorithm, EncryptionMetadata, KeyDerivationParams,
    SecureMemory, SecureRandom,
};
...
impl CryptoService {
    ...
    pub fn encrypt_data_csharp_compatible(
        &self,
        data: &[u8],
        key: &[u8],
        ...
        if data.is_empty() {
            return Err(
                CryptoError::InvalidData("암호화할 데이터가 비어있습니다.".to_string()).into(),
            );
        }
        ...
        if key.len() != 32 {
            return Err(
                CryptoError::InvalidKey("키는 32바이트(256비트)여야 합니다.".to_string()).into(),
            );
        }
    ...
    pub fn decrypt_data_csharp_compatible(
        &self,
        encrypted_data: &[u8],
        key: &[u8],
    ) -> SecureVaultResult<Vec<u8>> {
    ...
    pub fn decrypt_file(
        ...
        // C# 호환 형식으로 복호화
        let plaintext =
            self.decrypt_data_csharp_compatible(&encrypted_data.ciphertext, &file_key)?;
    ...
    fn encrypt_with_aes256gcm(
        &self,
        key: &[u8; 32],
        nonce: &[u8],
        data: &[u8],
    ) -> SecureVaultResult<(Vec<u8>, Vec<u8>)> {
        let cipher = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key));
    ...
    /// # 반환값
    /// * `SecureVaultResult<(Vec<u8>, Vec<u8>)>` - (암호문, 인증 태그)
    fn encrypt_with_chacha20poly1305(
        &self,
        key: &[u8; 32],
        nonce: &[u8],
        data: &[u8],
