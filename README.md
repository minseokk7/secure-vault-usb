...
### 5. � 복구 키 시스템 (Recovery Key)
- **24단어 복구 키**: 최초 설정 시 생성되는 복구 키를 안전한 곳에 보관하세요.
- **PIN 분실 대비**: 복구 키를 사용하여 PIN을 재설정할 수 있습니다.

### 6. 🎭 위장 모드 (Camouflage Mode)
- **계산기/메모장 위장**: 앱을 계산기나 메모장처럼 보이게 위장할 수 있습니다.
- **단축키 잠금해제**: 위장 상태에서 비밀 단축키로 실제 금고에 접근합니다.

### 7. �📝 보안 메모장 & 뷰어
- **Encrypted Memo**: 텍스트 메모를 암호화하여 DB에 안전하게 저장합니다.
- **In-Memory Viewer**: 이미지, 텍스트, PDF 파일을 디스크에 임시 파일로 저장하지 않고, **RAM에서 즉시 복호화하여 표시**합니다.
- **미디어 재생**: 동영상, 오디오 파일도 메모리에서 스트리밍 재생합니다.

### 8. 📊 감사 로그 (Audit Log)
- **접근 기록**: 모든 로그인 시도, 파일 접근, 설정 변경이 암호화된 로그에 기록됩니다.
- **침입 감지**: 실패한 로그인 시도를 추적하여 무차별 대입 공격을 감지합니다.

### 9. 🧹 시스템 흔적 정리 (System Cleanup)
- **자동 정리**: 앱 종료 시 운영체제의 최근 파일 목록, 임시 파일 등을 자동으로 정리합니다.
- **흔적 최소화**: 사용 흔적을 남기지 않습니다.
