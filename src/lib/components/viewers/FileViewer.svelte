...
<script lang="ts">
  ...
  // 파일 타입 감지 - 안전한 체크 추가
  $: viewerType = fileName ? getFileViewerType(fileName, file?.mime_type) : 'unsupported';
  
  // 미디어 파일 크기에 따른 모달 크기 결정
  $: modalSize = viewerType === 'media' ? 
    (file?.file_size && file.file_size > 50 * 1024 * 1024 ? 'large' : 'medium') : 'large';
  ...
  async function handleSave(content: string) {
    try {
      console.log('파일 저장 시작:', { file_id: file.id, contentLength: content.length });
      ...
      {:else if viewerType === 'image'}
        ...
        <div class="image-viewer">
          <div class="viewer-placeholder">
            ...
            <h3>이미지 뷰어</h3>
            <p>이미지 뷰어는 곧 구현될 예정입니다.</p>
            <p class="file-info">파일: {fileName}</p>
